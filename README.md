...
5年目車検前買替は車検・修理は少ない一方で、買替回数とローン期間が多くなります。9〜13年目車検前買替は車検・修理は増えますが、買替回数が少なくなります。

比較対象は `5年目車検前`、`7年目車検前`、`9年目車検前`、`11年目車検前`、`13年目車検前`、`最後まで維持・最終売却` です。表示上の年数は車検前買替年齢であり、実際の買替はその車検が発生する前年に前倒しされます。このため、短期買替では車検回数と高額イベントが少なくなる一方、買替回数とローン期間は増えます。

`最後まで維持・最終売却` は、評価期間中に買替を行わず、最後に売却して車利用を終了するシナリオです。47歳から約25年の車利用期間全体を見るうえで、買替なしの上限ケースとして使います。
